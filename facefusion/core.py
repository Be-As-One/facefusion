...
def apply_config(program : ArgumentParser) -> None:
	...
	# print("facefusion.globals.config_path:", facefusion.globals.config_path)
...
def apply_args(program : ArgumentParser) -> None:
	...
	# face selector
	if args.face_selector_mode in facefusion.choices.face_selector_modes:
	...
	if facefusion.globals.reference_frame_number < 0:
		...
		raise ValueError('Reference frame number must be greater than or equal to 0.')
	else:
		print("facefusion.globals.reference_frame_number:", facefusion.globals.reference_frame_number)
	...
	if is_image(args.target_path):
		# log.info(f"-----> processing image: {args.target_path}")
...
def preloads(program : ArgumentParser) -> None:
	validate_args(program)
	# print("-----------> apply_args", program)
	apply_args(program)
	...
	for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors):
		# print("----------start models pre check------------")
		if not frame_processor_module.pre_check():
			log.error(f"frame_processor_module pre_check failed: {frame_processor_module}")
			return
	# for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors):
	# 	print("--------start models post check------------")
	# 	t0 = time()
	# 	while not frame_processor_module.post_check() and time() - t0 < 100:
	# 		logger.disable()
	# 		sleep(0.5)
	# 	logger.enable()
...
def run(program : ArgumentParser) -> None:
	...
	if facefusion.globals.headless:
		...
		# print("-----------> facefusion.globals.ui_layouts:", facefusion.globals.ui_layouts)
...
def conditional_process() -> None:
	...
	for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors):
		while not frame_processor_module.post_check():
			logger.disable()
			sleep(0.5)
			log.error(f"frame_processor_module post_check failed: {frame_processor_module}")
		logger.enable()
		# if not frame_processor_module.pre_process('output'):
		# 	return
		result, error = frame_processor_module.pre_process('output')
		if not result:
	...
	if is_image(facefusion.globals.target_path):
		try:
			...
			if not is_image(facefusion.globals.output_path):
				raise ValueError('Output creation failed.')
		except Exception as e:
			log.error(f"process_image failed: {e}")
			raise e
	if is_video(facefusion.globals.target_path):
		try:
			process_video(start_time)
			if not is_video(facefusion.globals.output_path):
				raise ValueError('Output creation failed.')
		except Exception as e:
			log.error(f"process_video failed: {e}")
			raise e
...
def process_image(start_time : float) -> None:
	...
	log.info(f"normed_output_path: {normed_output_path}")
	# if analyse_image(facefusion.globals.target_path):
	# 	log.error(f"analyse_image failed target_path: {facefusion.globals.target_path}")
...
def process_video(start_time : float) -> None:
	...
	# 	log.error(f"analyse_video failed facefusion.globals.target_path: {facefusion.globals.target_path}")
	...
	if temp_frame_paths:
		...
		raise ValueError('video temporary frames not found.')
		# return
	# merge video
...
# 	process_image(time(), 'facefusion/tests/data/face.jpg', 'facefusion/tests/data/face_out.jpg', '640x640', ['facefusion/tests/data/face.jpg'], ['face_swapper'], False)
# 	process_video(time(), 'facefusion/tests/data/face.mp4', 'facefusion/tests/data/face_out.mp4', '640x640', 30, ['facefusion/tests/data/face.jpg'], ['face_swapper'], 0, 0, False)
