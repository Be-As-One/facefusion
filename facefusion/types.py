...
# ============================================================================
# Face Mapping Types for Reference Mode (Reference模式的人脸映射类型)
# ============================================================================
# 支持在reference模式下使用多个源脸-参考脸映射

# 单个人脸映射关系
FaceMapping = TypedDict('FaceMapping',
{
	'source_path': Optional[str],  # 源脸图片路径（要替换成的脸）
	'source_face': Optional[Face],  # 源脸对象（预加载）
	'reference_path': Optional[str],  # 参考脸图片路径（从视频中提取的要被替换的脸）
	'reference_face': Optional[Face],  # 参考脸对象（预加载）
	'similarity_threshold': Optional[float],  # 相似度阈值（可选，默认使用reference_face_distance）
	'enabled': bool  # 是否启用此映射
})

# 多映射配置集合
FaceMappingList : TypeAlias = List[FaceMapping]
