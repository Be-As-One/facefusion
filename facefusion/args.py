...
def apply_args(args : Args, apply_state_item : ApplyStateItem) -> None:
	...
	# ============================================================================
	# Face Mappings for Reference Mode (Reference模式的多人脸映射)
	# ============================================================================
	# 处理多参考脸配置文件
	face_mappings = None
	face_mappings_file = args.get('face_mappings_file')

	if face_mappings_file and os.path.exists(face_mappings_file):
		try:
			import json
			with open(face_mappings_file, 'r', encoding='utf-8') as f:
				config = json.load(f)
				face_mappings = config.get('mappings', [])
				logger.info(f'Loaded {len(face_mappings)} face mappings from {face_mappings_file}', __name__)
		except Exception as e:
			logger.error(f'Failed to load face mappings file: {e}', __name__)

	apply_state_item('face_mappings', face_mappings)
	apply_state_item('face_mappings_file', face_mappings_file)
	# ============================================================================
	# face masker
