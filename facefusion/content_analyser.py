...
def pre_check() -> bool:
	...
	# print("conntent analyser model_url:", model_url)
	model_path = MODELS.get('open_nsfw').get('path')
	# print("conntent analyser model_path:", model_path)

	if not facefusion.globals.skip_download:
		# print("conntent analyser not skip download")
		process_manager.check()
		conditional_download(download_directory_path, [ model_url ])
		process_manager.end()

	print("conntent analyser is file:", is_file(model_path))
...
def analyse_video(video_path: str, start_frame: int, end_frame: int) -> bool:
    ...
    logger.info(f"---------->analyse_video avg_fps: {avg_fps :.2f}, Time taken: {elapsed_time :.2f} seconds")
