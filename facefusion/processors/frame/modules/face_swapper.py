...
def _pre_process(mode : ProcessMode) -> bool:
	# logger.info("------------> pre_process face_swapper <-----------")
...
def pre_process(mode: ProcessMode) -> Tuple[bool, str]:
    # logger.info("------------> pre_process face_swapper <-----------")
    
    if not has_image(facefusion.globals.source_paths):
        error_message = wording.get('select_image_source') + wording.get('exclamation_mark')
    ...
    for source_frame in source_frames:
        if not get_one_face(source_frame):
            error_message = wording.get('no_source_face_detected') + wording.get('exclamation_mark')
            logger.error(error_message, NAME)
            return False, error_message

    if mode in ['output', 'preview'] and not (is_image(facefusion.globals.target_path) or is_video(facefusion.globals.target_path)):
        error_message = wording.get('select_image_or_video_target') + wording.get('exclamation_mark')
        logger.error(error_message, NAME)
        return False, error_message

    if mode == 'output' and not normalize_output_path(facefusion.globals.target_path, facefusion.globals.output_path):
        error_message = wording.get('select_file_or_directory_output') + wording.get('exclamation_mark')
        logger.error(error_message, NAME)
        return False, error_message
