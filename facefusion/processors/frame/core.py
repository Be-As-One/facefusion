...
def get_frame_processors_modules(frame_processors : List[str]) -> List[ModuleType]:
	print("get frame_processors:", frame_processors)
	...
	if not FRAME_PROCESSORS_MODULES:
		for frame_processor in frame_processors:
			...
			print("load frame_processor_modules:", len(FRAME_PROCESSORS_MODULES))
	return FRAME_PROCESSORS_MODULES


def clear_frame_processors_modules() -> None:
	global FRAME_PROCESSORS_MODULES
	print("clear frame_processors_modules:", len(FRAME_PROCESSORS_MODULES))
...
def _multi_process_frames(source_paths : List[str], temp_frame_paths : List[str], process_frames : ProcessFrames) -> None:
...
def multi_process_frames(source_paths: List[str], temp_frame_paths: List[str], process_frames: ProcessFrames) -> None:
    ...
    with ThreadPoolExecutor(max_workers=facefusion.globals.execution_thread_count) as executor:
        ...
        while not queue.empty():
            future = executor.submit(process_frames, source_paths, pick_queue(queue, queue_per_future), lambda x: None)
            futures.append(future)
        
        for future_done in as_completed(futures):
            future_done.result()
            processed_frames += queue_per_future
    
    total_time = time.time() - start_time
    average_fps = processed_frames / total_time if total_time > 0 else 0
    logger.info(f"Total time: {total_time:.2f} seconds, Average FPS: {average_fps:.2f}")



def create_queue(queue_payloads : List[QueuePayload]) -> Queue[QueuePayload]:
	# print("create queue_payloads:", len(queue_payloads))
